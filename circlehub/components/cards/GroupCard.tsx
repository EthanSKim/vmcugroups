...
const GroupCard = ({ data, tab, admin }: { data: GroupProps, tab:String, admin:boolean }) => {
  ...
  const handleJoin = async () => {
    try {
      ...
      if (userDataStr) {
        ...
        await axios.post("http://www.vmcugroupsapi.com/joinGroup", {userId, id});
      }
    } catch (err:any) {
  ...
  const handleLeave = async () => {
    try {
      ...
      if (userDataStr) {
        ...
        if (leave) {
          const userId = JSON.parse(userDataStr).id;
          await axios.post("http://www.vmcugroupsapi.com/leaveGroup", {userId, id});
        }
      }
    } catch (err) {
      console.log(err);
  ...
  const handleLock = async () => {
    try {
      await axios.post("http://www.vmcugroupsapi.com/lockGroup", {id});
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const handleUnlock = async () => {
    try {
      await axios.post("http://www.vmcugroupsapi.com/unlockGroup", {id});
    } catch(err) {
      console.log(err);
    }
  }
